...
def is_jibo_present():
    ...
    try:
        ...
        print(
            f"[  ] (@Linux/Debian:mH*jd) Critical Error: Failed to run lsusb, check usbutils are installed exiting..."
        )
        sys.exit(1)
...
def build_shofel():
    ...
    try:
        ...
        print(
            f"[  ] (@Linux/Debian:dH*jd) Critical Error: 'Make' failed without trying to compile Shofel, exiting.."
        )
        sys.exit(1)
    except FileNotFoundError:
        print(
            f"[  ] (@Linux/Debian:eH*jd) Critical Error: 'Make' wasnt found check 'build-essential' packages are installed, exiting.."
        )
        sys.exit(1)
...
def install_build_dependencies(missing_packages):
    ...
    print("[  ] Launching apt to fetch dependencies...     (root might be required)")
    ...
    try:
        ...
        print("[ 󱄌 ] Error : apt failed to install system dependencies")
        return False
...
def check_build_dependencies():
    ...
    for pkg in required_packages:
        result = subprocess.run(
            ["dpkg", "-l", pkg], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
        )
        if result.returncode != 0:
            missing_packages.append(pkg)
            print(
                f"[󰥨  ] Found missing package : {pkg} , it will get installed automatically later!"
            )
...
def load_msg():
    print("Debian/Ubuntu definitions!!")
