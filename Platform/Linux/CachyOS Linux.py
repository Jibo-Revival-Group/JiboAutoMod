...
def is_jibo_present():
    ...
    try:
        ...
        print(
            f"[  ] (@Linux/Cachy:mH*jd) Critical Error: Failed to run lsusb, check usbutils are installed exiting..."
        )
        sys.exit(1)
...
def build_shofel():
    ...
    try:
        ...
        print(
            f"[  ] (@Linux/Cachy:dH*jd) Critical Error: 'Make' failed without trying to compile Shofel, exiting.."
        )
        sys.exit(1)
    except FileNotFoundError:
        print(
            f"[  ] (@Linux/Cachy:eH*jd) Critical Error: 'Make' wasnt found check 'base-devel' packages are installed, exiting.."
        )
        sys.exit(1)
...
def install_build_dependencies(missing_packages):
    ...
    print(
        "[  ] Launching pacman to detch dependencies...     (root might be required)"
    )
...
def check_build_dependencies():
    ...
    for pkg in required_packages:
        result = subprocess.run(
            ["pacman", "-Qq", pkg], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
        )
